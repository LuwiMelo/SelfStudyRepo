...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("SELECT COUNT(StrandID) AS 'Total' FROM tblstrand");
    $statement->execute();
    $row = $statement->fetch();
...
try
{
    ...
    if (!empty($row)) {
            ...
            foreach($row as $dataa){
               ...
               $OutputData .= '["'.$dataa['StrandID'].'","'.$dataa['StrandName'].'","<button class=\"btn btn-success btnEditReligion\" data-id=\"'.$dataa['StrandID'].'\" data-name=\"'.$dataa['StrandName'].'\"  >Edit</button>","<button disabled class=\"btn btn-danger btnDeleteReligion\" data-id=\"'.$dataa['StrandID'].'\" data-name=\"'.$dataa['StrandName'].'\"    >Delete</button>"],';
                //echo $OutputData;
            }
