...
$DataTableQuery = "SELECT AdmissionID,GradeLevel, StudentID, StudentIDDisplay,CONCAT(LastName,' ',FirstName,' ',COALESCE(MiddleName,'')) AS 'Fullname', AdmissionReenrollmentID, AdmissionAssessmentIDDisplay FROM tblstudent,tblstudentadmission 
LEFT JOIN tblgradelevel ON tblstudentadmission.AdmissionGradeLevelId = tblgradelevel.GradeLevelID WHERE AdmissionStudentID = StudentID AND AdmissionSchoolYearID = $LatestSchoolYearID and ADMISSIONSTATUS = 2 AND AdmissionID IN(SELECT PaymentAdmissionID from tblpaymenttransactions WHERE PaymentSchoolYearID = $LatestSchoolYearID) GROUP BY StudentID ORDER BY Fullname ASC";
?>
...
<div id="content-header">
 ...
 <div class="container-fluid">
          ...
          <hr>
 <center><h1>List of Enrolled Students</h1></center>
...
try
{
    ...
    foreach($row as $data){
        ...
        echo '<td>'.$data['GradeLevel'].'</td>';
          ...
          echo " <td> <form action=\"CheckSOAInitializer.php\" method=\"post\" target=\"\"><input class=\"btn btn-primary\" type=\"submit\" value=\"Check SOA\" > <input type=\"hidden\" name=\"id\" value=\"".$data["StudentID"]."\">
