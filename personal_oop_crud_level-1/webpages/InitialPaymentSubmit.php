...
try
{
    ...
    if( $statement->execute(array(':TableName' => "tblpaymenttransactions", ':TableID' => $LastPaymentID, ':TableAction' => "INSERT", ':DateTime' => $DateToday,':TableUserID' => $_SESSION['SessionUserID'],':AuditDetails' => "CREATE INITIAL PAYMENT" ))  )
    {
...
//Check if student is re enrollee
...
try
{
    ...
    $statement = $dbh->prepare("SELECT AdmissionAssessmentIDDisplay FROM tblstudentadmission WHERE AdmissionID = :AdmissionID");
    $statement->execute(array(':AdmissionID' => $AdmissionID));
    $row = $statement->fetch();
    
    if (!empty($row)) {
          
        if(is_null($row['AdmissionAssessmentIDDisplay'])){
...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("UPDATE tblstudentadmission SET AdmissionReferenceNum = :AdmissionReferenceNum,AdmissionModeOfPaymentID = :AdmissionModeOfPaymentID, AdmissionStatus = :AdmissionStatus, AdmissionUpdate = :AdmissionUpdate WHERE AdmissionID = :AdmissionID");
    
    
    if( $statement->execute(array(':AdmissionReferenceNum' => $NewReferenceNum,':AdmissionModeOfPaymentID' => $Retrieve->ModeOfPayment,':AdmissionStatus' => 2, ':AdmissionUpdate' => 1, ':AdmissionID' => $AdmissionID ))  )
    {
