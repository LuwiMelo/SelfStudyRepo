...
$DataTableQuery = "SELECT GradeLevel, AdmissionReferenceNum,StudentIDDisplay,StudentID,AdmissionReenrollmentID,CONCAT(LastName,' ',FirstName,' ',COALESCE(MiddleName,'')) AS 'Fullname' FROM tblstudent,tblstudentadmission 
LEFT JOIN tblgradelevel ON tblstudentadmission.AdmissionGradeLevelId = tblgradelevel.GradeLevelID WHERE AdmissionStudentID = StudentID  AND StudentStatus = 0 AND LEFT(AdmissionReenrollmentID,4) = $LatestSchoolYear and ADMISSIONSTATUS = 2 AND LEFT(AdmissionReferenceNum,4) = $LatestSchoolYear GROUP BY StudentID ORDER BY StudentIDDisplay ASC";
?>
...
<div id="content-header">
 ...
 <div class="container-fluid">
          ...
          <hr>
 <center><h1>List of Re-Enrollees for Succeeding Payment</h1></center>
...
try
{
    ...
    foreach($row as $data){
        ...
        echo '<td>'.$data['GradeLevel'].'</td>';
          ...
          echo " <td> <form action=\"AddSucPaymentTransfereeInitializer.php\" method=\"post\"><input class=\"btn btn-primary\" type=\"submit\" value=\"Add New Payment\" > <input type=\"hidden\" name=\"id\" value=\"".$data["StudentID"]."\">
