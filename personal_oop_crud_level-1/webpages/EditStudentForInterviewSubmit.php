...
$AdmissionID = $_SESSION['EditStudentForInterviewAdmissionID'];
$EditStudentID = $_SESSION['VSFIStudentID'];    
...
try
{
    ...
    if( $statement->execute(array(':TableName' => "tblstudent", ':TableID' => $EditStudentID, ':TableAction' => "UPDATE", ':DateTime' => $DateToday,':TableUserID' => $_SESSION['SessionUserID'],':AuditDetails' => "UPDATED TRANSFEREE PERSONAL RECORD" ))  )
    {
...
try
{
    ...
    /*
    $statement = $dbh->prepare("UPDATE tblstudentadmission SET InterviewDate = :InterviewDate, AdmissionSchoolOfOrigin = :AdmissionSchoolOfOrigin, AdmissionTypeOfInstitution = :AdmissionTypeOfInstitution, AdmissionGradeLevelID = :AdmissionGradeLevelID WHERE AdmissionID = :AdmissionID ");
    
    
    if( $statement->execute(array(':InterviewDate' => $Retrieve->InterviewDateInsert, ':AdmissionSchoolOfOrigin' => $Retrieve->AdmissionSchoolOfOrigin,':AdmissionTypeOfInstitution' => $Retrieve->AdmissionTypeOfInstitution, ':AdmissionGradeLevelID' => $Retrieve->IncomingGradeLevel, ':AdmissionID' => $AdmissionID ))  )
    {
...
try
{
    ...
    if( $statement->execute(array(':TableName' => "tblstudentadmission", ':TableID' => $AdmissionID, ':TableAction' => "UPDATE", ':DateTime' => $DateToday,':TableUserID' => $_SESSION['SessionUserID'], ':AuditDetails' => "UPDATED TRANSFEREE ADMISSION RECORD" ))  )
    {
...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("DELETE FROM tblsibling WHERE SiblingTableAdmissionID = :SiblingTableAdmissionID AND ExternalSiblingName IS NULL");
