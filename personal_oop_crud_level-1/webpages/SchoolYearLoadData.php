...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("SELECT COUNT(SchoolYearID) AS 'Total' FROM tblschoolyear");
    $statement->execute();
    $row = $statement->fetch();
...
try
{
    ...
    $statement = $dbh->prepare("SELECT * FROM tblschoolyear");
    $statement->execute();
    ...
    if (!empty($row)) {
            ...
            foreach($row as $dataa){
               ...
               $OutputData .= '["'.$dataa['SchoolYearID'].'","'.$dataa['SchoolYear'].'","<button class=\"btn btn-success btnEditNationality\" data-id=\"'.$dataa['SchoolYearID'].'\" data-name=\"'.$dataa['SchoolYear'].'\"  >Edit</button>","<button class=\"btn btn-danger btnDeleteNationality\" data-id=\"'.$dataa['SchoolYearID'].'\" data-name=\"'.$dataa['SchoolYear'].'\"    >Delete</button>"],';
                //echo $OutputData;
            }
