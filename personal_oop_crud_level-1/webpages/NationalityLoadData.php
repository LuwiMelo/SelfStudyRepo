...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("SELECT COUNT(NationalityID) AS 'Total' FROM tblnationality");
    $statement->execute();
    $row = $statement->fetch();
...
try
{
    ...
    if (!empty($row)) {
            ...
            foreach($row as $dataa){
               ...
               $OutputData .= '["'.$dataa['NationalityID'].'","'.$dataa['NationalityName'].'","<button class=\"btn btn-success btnEditNationality\" data-id=\"'.$dataa['NationalityID'].'\" data-name=\"'.$dataa['NationalityName'].'\"  >Edit</button>","<button disabled class=\"btn btn-danger btnDeleteNationality\" data-id=\"'.$dataa['NationalityID'].'\" data-name=\"'.$dataa['NationalityName'].'\"    >Delete</button>"],';
                //echo $OutputData;
            }
