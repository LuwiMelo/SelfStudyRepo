...
if (isset($_POST["RetrieveTransaction"]))
{
 ...
 $Phase2AdmissionID = $_SESSION['UpdateInterviewResultAdmissionID'];
 $Phase2StudentID  = $_SESSION['UpdateInterviewResultStudentID'];
 $Phase2AdmissionPhase2ID = $_SESSION['UpdateInterviewResultAdmissionPhase2ID'];
...
try
{
    ...
    $statement = $dbh->prepare("UPDATE tbladmissionphase2 SET AdmissionStudentAdmissionID =  :AdmissionStudentAdmissionID,AdmissionPhase2StudentNickname = :AdmissionPhase2StudentNickname,PersonalInfoRemarks =  :PersonalInfoRemarks,FacebookMessenger = :FacebookMessenger,FamilyBackgroundRemarks =  :FamilyBackgroundRemarks,ExtendedFamilyBackgroundRemarks = :ExtendedFamilyBackgroundRemarks, AcademicStrengths = :AcademicStrengths, AcademicWeaknesses = :AcademicWeaknesses, RecognitionReceived = :RecognitionReceived, OtherInterests =  :OtherInterests, SpecialNeeds = :SpecialNeeds, SeriousDisciplinaryCase = :SeriousDisciplinaryCase, ConcernsForTransfer = :ConcernsForTransfer, ReasonsForTransfer = :ReasonsForTransfer, WhoDecidesTheTransfer = :WhoDecidesTheTransfer, EducationalBackgroundRemarks = :EducationalBackgroundRemarks, ChurchName = :ChurchName, ChurchLocation = :ChurchLocation, ChurchEngagement = :ChurchEngagement, SchoolRelatedClubs = :SchoolRelatedClubs, OtherOrgInvolvement = :OtherOrgInvolvement, OrganizationsRemarks = :OrganizationsRemarks, RateOfGeneralHealth = :RateOfGeneralHealth, 	HearingCondition = :HearingCondition, EyesightCondition = :EyesightCondition, Allergies = :Allergies, OtherHealthConcerns = :OtherHealthConcerns, HealthCategoryRemarks = :HealthCategoryRemarks, ServicesOfInterests = :ServicesOfInterests, AncillaryRemarks = :AncillaryRemarks WHERE AdmissionPhase2ID = :AdmissionPhase2ID ");
    
    
    
    
    if ($statement->execute(array(':AdmissionStudentAdmissionID' => $Phase2AdmissionID , ':AdmissionPhase2StudentNickname' => $Retrieve->Phase2Nickname, ':PersonalInfoRemarks' => $Retrieve->Phase2PersonalInfoRemarks, ':FacebookMessenger' => $Retrieve->Phase2FacebookMessenger, ':FamilyBackgroundRemarks' => $Retrieve->FamilyBackgroundRemarks, ':ExtendedFamilyBackgroundRemarks' => $Retrieve->ExtendedFamilyBackgroundRemarks, ':AcademicStrengths' => $Retrieve->AcademicStrengths, ':AcademicWeaknesses' => $Retrieve->AcademicWeaknesses, ':RecognitionReceived' => $Retrieve->RecognitionsReceived, ':OtherInterests' => $Retrieve->OtherInterests, ':SpecialNeeds' => $Retrieve->SpecialNeeds, ':SeriousDisciplinaryCase' => $Retrieve->DisciplinaryCase, ':ConcernsForTransfer' => $Retrieve->ConcernsForTransfer, ':ReasonsForTransfer' => $Retrieve->ReasonsForChoosing, ':WhoDecidesTheTransfer' => $Retrieve->WhoDecides, ':EducationalBackgroundRemarks' => $Retrieve->EducBackgroundRemarks, ':ChurchName' => $Retrieve->ChurchName, ':ChurchLocation' => $Retrieve->ChurchLocation, ':ChurchEngagement' => $Retrieve->ChurchEngagement, ':SchoolRelatedClubs' => $Retrieve->SchoolClubs,  ':OtherOrgInvolvement' => $Retrieve->OtherOrg, ':OrganizationsRemarks' => $Retrieve->OrgBackgroundRemarks,  ':RateOfGeneralHealth' => $Retrieve->RateOfGeneralHealth,  ':HearingCondition' => $Retrieve->HearingCondition, ':EyesightCondition' => $Retrieve->EyesightCondition,  ':Allergies' => $Retrieve->Allergies, ':OtherHealthConcerns' => $Retrieve->OtherHealthConcerns, ':HealthCategoryRemarks' => $Retrieve->HealthBackgroundRemarks, ':ServicesOfInterests' => $Retrieve->ServicesOfInterest,  ':AncillaryRemarks' => $Retrieve->AncillaryRemarks, ':AdmissionPhase2ID' => $Phase2AdmissionPhase2ID  ))             )
...
$LastAdmissionPhase2ID = $Phase2AdmissionPhase2ID ;
...
try
{
    ...
    if( $statement->execute(array(':TableName' => "tbladmissionphase2", ':TableID' => $Phase2AdmissionPhase2ID, ':TableAction' => "UPDATED", ':DateTime' => $DateToday,':TableUserID' => $_SESSION['SessionUserID'], ':AuditDetails' => "UPDATED INTERVIEW  RESULT" ))  )
    {
...
try
{
    ...
    if( $statement->execute(array(':TableName' => "tblstudentadmission", ':TableID' => $Phase2AdmissionID, ':TableAction' => "UPDATE", ':DateTime' => $DateToday,':TableUserID' => $_SESSION['SessionUserID'],':AuditDetails' => "UPDATED STUDENT ADMISSION DISCOUNTS" ))  )
    {
...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("DELETE FROM tblotherschoolsattended WHERE OSAAdmissionPhase2ID = :OSAAdmissionPhase2ID");
    if ($statement->execute(array(':OSAAdmissionPhase2ID' => $LastAdmissionPhase2ID))  ){
        // success
...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("DELETE FROM tblenrollmentreferrals WHERE ReferralAdmissionPhase2ID = :ReferralAdmissionPhase2ID");
    if ($statement->execute(array(':ReferralAdmissionPhase2ID' => $LastAdmissionPhase2ID))  ){
        // success
...
{
    //$LatestSchoolYear;
    $statement = $dbh->prepare("DELETE FROM tblexclusiverelationship WHERE ERAdmissionPhase2ID = :ERAdmissionPhase2ID");
    if ($statement->execute(array(':ERAdmissionPhase2ID' => $LastAdmissionPhase2ID))  ){
        // success
