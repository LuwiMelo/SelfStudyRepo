...
if($_POST['ModeOfPaymentFrom'] == $_POST['ModeOfPaymentTo']){
    
    $StrandQuery = " AND COALESCE(AdmissionModeOfPaymentID,'') = ".$_POST['ModeOfPaymentFrom'];
}
else{
    $StrandQuery = " AND COALESCE(AdmissionModeOfPaymentID,'') BETWEEN ".$_POST['ModeOfPaymentFrom']." AND ".$_POST['ModeOfPaymentTo'];
}


$DataTableQuery = "SELECT StrandName,StudentIDDisplay,LastName,FirstName,COALESCE(MiddleName,'') AS 'Middle',COALESCE(ESCNumber,'') AS 'ESC', COALESCE(SectionName,'') AS 'Section', COALESCE(AdmissionModeOfPaymentID,'') AS 'ModeOfPayment', PaymentOptionName FROM tblstudent LEFT JOIN tblstrand ON tblstudent.StudentStrand = tblstrand.StrandID,tblstudentadmission LEFT JOIN tblsection ON tblsection.SectionID = tblstudentadmission.AdmissionSectionID,tblpaymentoption WHERE AdmissionStudentID = StudentID AND AdmissionReferenceNum = $LatestSchoolYear AND AdmissionModeOfPaymentID = PaymentOptionID AND LEFT(AdmissionReferenceNum,4) = $LatestSchoolYear and ADMISSIONSTATUS = 2 ";
...
try
{
    ...
    foreach($row as $data){
        ...
        echo '<td>'.$data['PaymentOptionName'].'</td>';
